...
uint64 FCEU_fread(void *ptr, size_t size, size_t nmemb, FCEUFILE *fp)
{
 if(fp->type==1)
 {
  return gzread((gzFile)fp->fp,ptr,size*nmemb);
 }
 else if(fp->type>=2)
 {
  MEMWRAP *wz;
...
int FCEU_fseek(FCEUFILE *fp, long offset, int whence)
{
 if(fp->type==1)
 {
  return( (gzseek((gzFile)fp->fp,offset,whence)>0)?0:-1);
 }
 else if(fp->type>=2)
 {
  MEMWRAP *wz;
...
int FCEU_read16le(uint16 *val, FCEUFILE *fp)
{
 ...
 if(fp->type>=1)
 {
   ...
   *(uint32 *)t=*(uint32 *)(wz->data+wz->location);
   ...
   if(gzread((gzFile)fp->fp,&t,2)!=2) return(0);
  return(1);
 }
 else
 {
...
uint64 FCEU_fgetsize(FCEUFILE *fp)
{
 ...
 {
  int x,t;
  t=gztell((gzFile)fp->fp);
  gzrewind((gzFile)fp->fp);
  for(x=0; gzgetc((gzFile)fp->fp) != EOF; x++);
  gzseek((gzFile)fp->fp,t,SEEK_SET);
  return(x);
 }
 else if(fp->type>=2)
